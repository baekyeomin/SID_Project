...
# parquet에서 읽은 값을 Python list로 변환
...
class NewsSequenceDataset(Dataset):
    ...
    def __init__(
        ...
        # Transformer 학습에 반드시 필요한 column 확인
        required_columns = [
            "history_c1",
            "history_c2",
            "history_c3",
    ...
    def _build_sample_index(self) -> None:
        ...
        for row_idx, value in enumerate(self.df["history_c1"]):
            history_c1 = _to_int_list(value)

            # 혹시 history가 없는 사용자가 있을 경우 제외
    ...
    def __getitem__(self, index: int,) -> Dict[str, Any]:
        ...
        if self.validate_data:
            ...
            # 반드시 5개 후보인지 확인
            if (candidate_lengths[0] != NUM_CANDIDATES):
            ...
            # (c1,c2,c3)가 candidate 안에서 서로 겹치지 않는지 확인
            ...
            if (len(set(candidate_triplets)) != NUM_CANDIDATES):
        ...
        # 4. SID 값 검증
        if self.validate_data:
            if (history_sids.numel() > 0 and (history_sids < 0).any()):
                raise ValueError(
        ...
        if "history_article_ids" in self.df.columns:
            history_article_ids = _to_list(row["history_article_ids"])

            if self.max_history_length is not None:
                history_article_ids = (history_article_ids[-self.max_history_length:])

            if (self.validate_data and len(history_article_ids) != history_length):
                raise ValueError(
        ...
        if "target_article_ids" in self.df.columns:
            target_article_ids = _to_list(row["target_article_ids"])

        # Candidate article IDs
        candidate_article_ids = None

        if "candidate_article_ids" in self.df.columns:
            candidate_article_ids = _to_list(row["candidate_article_ids"])

            if (self.validate_data and len(candidate_article_ids) != NUM_CANDIDATES):
                raise ValueError(
...
# Collate function
def collate_news_sequences(batch: List[Dict[str, Any]],) -> Dict[str, Any]:
    if len(batch) == 0:
    ...
    # [B, max_history, 4]
    history_sids = torch.full((batch_size, max_history_length, NUM_HISTORY_SID_LEVELS,), fill_value=PAD_SID_VALUE, dtype=torch.long,)

    # [B, max_history]
    history_mask = torch.zeros((batch_size, max_history_length,), dtype=torch.bool,)
    ...
    for batch_idx, sample in enumerate(batch):
        ...
        history_sids[batch_idx, :seq_len, :,] = seq

        history_mask[batch_idx,:seq_len,] = True

    history_lengths_tensor = torch.tensor(history_lengths, dtype=torch.long,)

    # 2. Candidate
    # 모든 sample이 이미 후보 5개이므로
    # candidate은 padding이나 mask가 필요 X
    candidate_sids = torch.stack([sample["candidate_sids"] for sample in batch], dim=0,) # [B, 5, 3]
    candidate_labels = torch.stack([sample["candidate_labels"] for sample in batch], dim=0,) # [B, 5]

    # 3. Metadata
    impression_ids = [sample["impression_id"] for sample in batch]
    user_ids = [sample["user_id"] for sample in batch]
    impression_times = [sample["impression_time"] for sample in batch]
    history_article_ids = [sample["history_article_ids"] for sample in batch]
    target_article_ids = [sample["target_article_ids"] for sample in batch]
    candidate_article_ids = [sample["candidate_article_ids"] for sample in batch]
...
# Simple test (실제 학습 X / 코드 정상 작동 확인용)
